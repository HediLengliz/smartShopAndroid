...
const runMigrations = async (pool) => {
  try {
    ...
    // Migration 1: Add is_favorite column to orders table
    try {
      ...
      if (tables.length === 0) {
        console.log('  - Orders table does not exist yet, skipping migration');
        return;
      }
      ...
      if (columns.length === 0) {
        // Check if tracking_number column exists to determine position
        const [trackingColumn] = await pool.execute(
          `SELECT COLUMN_NAME 
           FROM INFORMATION_SCHEMA.COLUMNS 
           WHERE TABLE_SCHEMA = ? 
           AND TABLE_NAME = 'orders' 
        ...
        const alterStatement = trackingColumn.length > 0
          ? `ALTER TABLE orders ADD COLUMN is_favorite BOOLEAN DEFAULT FALSE AFTER tracking_number`
          : `ALTER TABLE orders ADD COLUMN is_favorite BOOLEAN DEFAULT FALSE`;

        await pool.execute(alterStatement);
        console.log('  ✓ Added is_favorite column to orders table');
      } else {
        console.log('  - Column is_favorite already exists in orders table');
      }
    } catch (error) {
      // Handle specific MySQL error codes
      if (error.code === 'ER_DUP_FIELDNAME') {
        console.log('  - Column is_favorite already exists (duplicate field error)');
    ...
    // Don't throw - migrations are optional, allow server to continue
  }
};
