...
class _ShoppingListDetailScreenState extends State<ShoppingListDetailScreen> {
  ...
  Future<void> _addListToCart() async {
    try {
      if (_items.isEmpty) {
        _showError('Shopping list is empty');
        return;
      }
        ...
        SnackBar(
          content: Row(
            children: [
              ...
              SizedBox(width: 8),
              Expanded(
                child: Text('Added ${_items.length} item${_items.length > 1 ? 's' : ''} to cart'),
              ),
            ],
          ),
      ...
      _showError('Failed to add items to cart: $e');
    }
  }
  ...
  Widget _buildSearchResults() {
    ...
    return Column(
      ...
      children: [
        ...
        Column(
          mainAxisSize: MainAxisSize.min,
          children: sortedResults.map((scoredProduct) => _buildScoredSearchResultItem(scoredProduct)).toList(),
        ),
        SizedBox(height: 16),
      ],
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // Add to Cart button in app bar - always visible when items exist
          if (!_showSearch && _items.isNotEmpty)
            Container(
              margin: EdgeInsets.only(right: 8),
              child: ElevatedButton.icon(
                onPressed: _addListToCart,
                      ...
                      ? SingleChildScrollView(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              // Template Suggestion
                              if (_detectedTemplate != null) _buildTemplateSuggestion(),

                              // Search Results
                              if (_scoredResults.isNotEmpty) _buildSearchResults(),
                            ],
                          ),
                        )
                      : _items.isEmpty
      ...
      floatingActionButton: !_showSearch && _items.isNotEmpty
